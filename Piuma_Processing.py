...
def hertzian_contact_func(x, E, R, x_0):
    return 4/3 * E * R**(1/2) * np.maximum(0, (x - x_0))**(3/2)
            ...
            with open(file_path, 'r') as f:
                for line in f:
                    if read_flag == 1:
                        line = line.strip()
                        parts = line.split('\t')
                        crosshead.append(float(parts[2]))  # Convert to float
                        load.append(float(parts[1]))  # Convert to float
                        times.append(float(parts[0]))  # Convert to float
                    if line == "Time (s)	Load (uN)	Indentation (nm)	Cantilever (nm)	Piezo (nm)	Auxiliary\n":
                        read_flag = 1
            data_records.append({
                "filename": filename,
                "crosshead": crosshead,
...
class Data:
    ...
    def Strain_Stress_graph(self):
        ...
        if self.test_type == 'sc':
            fit_start, fit_end, popt = self.single_comp()
        x = self.crosshead
        y = self.load
        fig, ax = plt.subplots()
        ...
        if self.test_type == 'dc':
            ...
            return plasticity
        if self.test_type == 'sc':
            index_closest = np.abs(x - popt[1]).argmin()
            x_half = x[:np.argmax(x)]
            x_fit = x_half[index_closest:]
            y_fit = hertzian_contact_func(x_fit,popt[0], self.radius, popt[1])
            ax.plot(x_fit, y_fit, 'r', markersize=5)
            with open(os.path.join(folder_path, self.name + ".pkl"), 'wb') as file:
                pickle.dump(fig, file)
            plt.close(fig)
            return popt[0]*0.75*10**12
        with open(os.path.join(folder_path, self.name + ".pkl"), 'wb') as file:
            pickle.dump(fig, file)
        plt.close(fig)
    ...
    def single_comp(self):
        ...
        index_max = np.argmax(self.load)
        x = self.crosshead[:index_max]
        y = self.load[:index_max]
        linear_e, intercept, r_value, p_value, std_err = stats.linregress(x, y)
        min_x_0 = np.min(x[:len(x) // 2])
        initial_guess = [linear_e,min_x_0]
        bounds = ([-np.inf, min_x_0], [np.inf, min_x_0+1000])
        popt, pcov = curve_fit(lambda x, E, x_0:hertzian_contact_func(x, E, self.radius, x_0), x, y, p0 = initial_guess, bounds=bounds)
        return 0, len(self.crosshead), popt
...
def sig_run(area, total_n, t_ftpr, filename, contr, plasticity, dc_name, modulus, half_time, sc_name, all_test_name):
    ...
    if ftpr[len(ftpr) - 1] == 'c' and ftpr[len(ftpr) - 2] == 'd':
        ...
        test_type = 'sc'
    # Call read_file to get data from multiple files
    data_records = read_file(ftpr)  # 'ftpr' is the folder path
    # Loop through the data records and create Data objects
    rep = 0
    for record in data_records:
        c, l, t = record["crosshead"], record["load"], record["times"]
        zero_indices = [i for i, x in enumerate(t) if x == 0]
        start = zero_indices[rep]
        end = zero_indices[rep + 1] if rep + 1 < len(zero_indices) else len(t)
        c = c[start:end]
        l = l[start:end]
        t = t[start:end]
        rep = rep + 1
        filename = record["filename"]  # Extract the filename for reference
        # Create a Data object for each file's data
        data_obj = Data(c, l, t, test_type, 0, t_area, filename, t_ftpr)

        if test_type == 'dc':
            plasticity.append(data_obj.Strain_Stress_graph())
            hal = data_obj.time_load_graph()
            dc_name.append(filename)
        else:
            slop = data_obj.Strain_Stress_graph()
            modulus.append(slop)
            hal = data_obj.time_load_graph()
        ...
        data_obj.time_load_graph()
        data_obj.time_crosshead_graph()
        data_obj.crosshead_load_graph()
...
def save_values():
    ...
    all_files = [x for x in all_files if x != 'Figure' and x != "default" and x != "Calib"]
...
entry1.pack()
label = tk.Label(top_frame, text="Enter radius (nm):")
label.pack()
